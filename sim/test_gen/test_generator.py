...
"""
This Python code generates necessary test files for polynomial
...
for PE_NUMBER, FILE_NAME in zip(PE_NUMBER_LIST, FILE_NAME_LIST):
    ...
    def CRT_Iterative_NWC_FD2_NR(A, w, q, TESTEN=True, TESTNUM=""):
        ...
        if TESTEN:
            KYBER_DIN0_MFNTT_TXT_DEBUG = open(
                FILE_NAME+"/debug/KYBER_"+TESTNUM+"_MFNTT_DEBUG.txt", "w")
        ...
        while lena >= 2:
            if TESTEN:
                KYBER_DIN0_MFNTT_TXT_DEBUG.write(
                    "********************** STAGE START **********************\n")
            ...
            while start < N:
                ...
                while (j < (start + lena)):
                    ...
                    if VERBOSE:
                        print("W: "+str(W_pow).ljust(5)+" A0: " +
                              str(j).ljust(5)+" A1: "+str(j+lena).ljust(5))

                    if TESTEN:
                        KYBER_DIN0_MFNTT_TXT_DEBUG.write("(A["+str(j).ljust(4)+"]: "+str((hex(bt1)[2:]).rstrip("L")).ljust(12)+" A["+str(j+lena).ljust(4)+"]: "+str((hex(bt2)[2:]).rstrip("L")).ljust(
                            12)+" W["+str(W_pow).ljust(4)+"]: "+str((hex(W)[2:]).rstrip("L")).ljust(12)+") -> ("+str((hex(B[j])[2:]).rstrip("L")).ljust(12)+" "+str((hex(B[j+lena])[2:]).rstrip("L")).ljust(12)+")"+"\n")

                    BRAM[(2*(bram_counter >> 0) & (PE-1))+0][bram_counter //
                                                             (N//2)][(bram_counter & ((N//2)-1)) // (PE//2)] = j
                    BRAM[(2*(bram_counter >> 0) & (PE-1))+1][bram_counter //
                                                             (N//2)][(bram_counter & ((N//2)-1)) // (PE//2)] = j+lena

                    BRTW[bram_counter & (
                        (PE//2)-1)][bram_counter // (N//2)][(bram_counter & ((N//2)-1)) // (PE//2)] = W_pow
    ...
    def ICRT_Iterative_NWC_FD2_RN(A, w, q, TESTEN=True, TESTNUM=""):
        ...
        if TESTEN:
            KYBER_DOUT_MINTT_TXT_DEBUG = open(
                FILE_NAME+"/debug/KYBER_"+TESTNUM+"_MINTT_DEBUG.txt", "w")
        ...
        while lena <= (N//2):
            if TESTEN:
                KYBER_DOUT_MINTT_TXT_DEBUG.write(
                    "********************** STAGE START **********************\n")
            ...
            while start < N:
                ...
                while (j < (start + lena)):
                    ...
                    if TESTEN:
                        KYBER_DOUT_MINTT_TXT_DEBUG.write("(A["+str(j).ljust(4)+"]: "+str((hex(bt1)[2:]).rstrip("L")).ljust(12)+" A["+str(j+lena).ljust(4)+"]: "+str((hex(bt2)[2:]).rstrip("L")).ljust(
                            12)+" W["+str(W_pow).ljust(4)+"]: "+str((hex(TW)[2:]).rstrip("L")).ljust(12)+") -> ("+str((hex(B[j])[2:]).rstrip("L")).ljust(12)+" "+str((hex(B[j+lena])[2:]).rstrip("L")).ljust(12)+")"+"\n")
    ...
    def HalfModPolMul(A, B_ntt, w, w_inv, q):
        # print("---- NTT(A)")
        N = len(A)
        A_ntt, ABR, ATW = CRT_Iterative_NWC_FD2_NR(
            A, w, q, TESTEN=False, TESTNUM="DIN0")

        C_ntt = [0 for _ in range(N)]

        if VERBOSE:
            print("---- Coefficient-wise multiplication:")
        KYBER_DOUT_COEFMUL_TXT_DEBUG = open(
            FILE_NAME+"/debug/KYBER_DOUT_COEFMUL_DEBUG.txt", "w")
        # Degree-2 modular polynomial multiplications
        for i in range(N//2):
            w_pow = 2*intReverse(i, int(log(N//2, 2)))+1
            if VERBOSE:
                print("W: "+str(w_pow).ljust(5) +
                      " A: {}".format(range(2*i, 2*i+2)))

            wk = pow(w, w_pow, q)
            C_ntt[2*i:2*i +
                  2] = PolWiseMult(A_ntt[2*i:2*i+2], B_ntt[2*i:2*i+2], wk, 2, q)
        ...
        if VERBOSE:
            print("---- INTT(C)")
        C, CBR, CTW = ICRT_Iterative_NWC_FD2_RN(
            C_ntt, w_inv, q, TESTEN=True, TESTNUM="DOUT")
    ...
    if VERBOSE:
        print("\n-------- Addressing for NTT --------")
    A_ntt, ABR, ATW = CRT_Iterative_NWC_FD2_NR(
        A, w, q, TESTEN=True, TESTNUM="DIN0")
    if VERBOSE:
        print("\n-------- Addressing for NTT --------")
    B_ntt, BBR, BTW = CRT_Iterative_NWC_FD2_NR(
        B, w, q, TESTEN=True, TESTNUM="DIN1")

    if VERBOSE:
        print("\n-------- Addressing for INTT --------")
    A_rec, NBR, NTW = ICRT_Iterative_NWC_FD2_RN(
        A_ntt, wv, q, TESTEN=True, TESTNUM="DIN0")
    if VERBOSE:
        print("\n-------- Addressing for INTT --------")
    B_rec, NBR, NTW = ICRT_Iterative_NWC_FD2_RN(
        B_ntt, wv, q, TESTEN=True, TESTNUM="DIN1")
    ...
    def PrintBRAM(BRAM, ring=0, findeg=1):
        ...
        for j in range(v):
            ...
            for k in range(m):
                BS = BS + "AD"+str(k)+":"
                for i in range(PE//2):
                    BS = BS+"\t|"+str(BRAM[2*i][j][k]).ljust(5) + \
                        str(BRAM[2*i+1][j][k]).ljust(4)+"|"
                BS = BS+"\n"
    ...
    # Data
    KYBER_BR_TXT = open(FILE_NAME+"/mem/KYBER_mem_N" +
                        str(n)+"_PE"+str(PE_NUMBER)+".txt", "w")
    KYBER_BR_TXT.write(
        "---------------------------------------------------------------------- Forward NTT (x2)\n")
    KYBER_BR_TXT.write(KYBERF_BR)
    KYBER_BR_TXT.write(
        "---------------------------------------------------------------------- Degree-2 polynomial-wise multiplication\n")
    KYBER_BR_TXT.write(
        "---------------------------------------------------------------------- Inverse NTT\n")
    KYBER_BR_TXT.write(KYBERI_BR)
    KYBER_BR_TXT.close()
    # Twiddle
    KYBER_TW_TXT = open(FILE_NAME+"/mem/KYBER_tw_N" +
                        str(n)+"_PE"+str(PE_NUMBER)+".txt", "w")
    KYBER_TW_TXT.write(
        "---------------------------------------------------------------------- Forward NTT (x2)\n")
    ...
    for j in range(v):
        for k in range(0, m, max(1, m >> j)):
            for i in range(PE//2):
                KYBER_W_TXT.write(
                    hex(pow(w, N0TW[i][j][k], q)).replace("L", "")[2:]+"\t  ")
            KYBER_W_TXT.write("\n")
    v = int(log(n//2, 2))
    m = n//PE
    for j in range(v):
        for k in range(0, m, max(1, m >> (v-j-1))):
            for i in range(PE//2):
                KYBER_WINV_TXT.write(
                    hex(pow(wv, N1TW[i][j][k], q)).replace("L", "")[2:]+"\t  ")
            KYBER_WINV_TXT.write("\n")
